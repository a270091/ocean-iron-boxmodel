...
modelrun = '0L';         % 0L or generic (makes no difference, though)
if strcmp(modelrun,'0L')
    % settings corresponding to run 0L in Frontiers paper
    fprintf('Parameter setting: 0L\n');
    params = load('results/parameters_0l.mat');
else
...
%-----------------------------
% initialize model concentrations
%-----------------------------

% initial PO4 distribution from WOA
...
%-----------------------------
% integrate model into steady state
%-----------------------------
...
% analyze sources and sinks of iron in every box, using final steady state
dconcdt = boxmodel_dgl_po4dopfe_export(0,finalstate);

fprintf('   Fe sources and sinks: (10^6 mol/yr) \n');
for k=1:12
    fprintf('Box %i: %s\n',k,params.long_names{k});
    fprintf('  Uptake: %6.1f \n',rhs.uptake(k)*params.volume(k)*1.0e-12);
    fprintf('  Remin:  %6.1f \n',rhs.remin(k)*params.volume(k)*1.0e-12);
    fprintf('  Scav:   %6.1f \n',rhs.scav(k)*params.volume(k)*1.0e-12);
    fprintf('  Dust:   %6.1f \n',rhs.dust(k)*params.volume(k)*1.0e-12);
    fprintf('  Sed:    %6.1f \n',rhs.sedfe(k)*params.volume(k)*1.0e-12);
    fprintf('  Hydro:  %6.1f \n',rhs.hydro(k)*params.volume(k)*1.0e-12);
end
